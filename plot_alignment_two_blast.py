...
# plot the alignment results for two blastn file
# 2017.04.04

loc_file1=sys.argv[1]
loc_file2=sys.argv[2]
...
loc_dict1 = read_loc_file(loc_file1)
loc_dict2 = read_loc_file(loc_file2)

## plot 
plot_genome_name = 'HXB2_9719:'
#plt.figure(figsize=(30,8))
plt.figure(figsize=(15, 18))
loc_list = []

for genome in loc_dict1:
    if genome==plot_genome_name:
        loc_list.append(loc_dict1[genome])
for genome in loc_dict2:
    if genome==plot_genome_name:
        loc_list.append(loc_dict2[genome])
#pdb.set_trace()

index = 0
for contig_list in loc_list:
    index+=1
    plt.subplot(len(loc_list),1, index)
    #contig_list=loc_dict[genome]
    con_idx=0
    print contig_list
    contig_list = sorted(contig_list, key=lambda x:x[1][1], reverse=True)
    ...
    plt.xlabel('HXB2 genome (bp)', fontsize=10, fontweight='bold')
    ...
    if index==1:
        plt.title('PEHaplo', fontsize = 12, fontweight='bold')
    else:
        plt.title('SAVAGE', fontsize = 12, fontweight='bold')
    ...
    for con in contig_list:
        ...
        plt.plot((con_align_start,con_align_end),(0.2,0.2),'g-',linewidth=4.0)  # plot on the genome
        plt.text(con_align_start-200,con_idx+0.2,str(con_al)+'('+str(con_align_start)+')',horizontalalignment='center',verticalalignment='center',fontsize=8,color='g') # left coordinate
        plt.text(con_align_end+200,con_idx+0.2,str(con_ar)+'('+str(con_align_end)+')',horizontalalignment='center',verticalalignment='center',fontsize=8) # right coordinate
        plt.text(con_align_mid-50,con_idx+0.2, str(simi)+'%',horizontalalignment='center',verticalalignment='center',fontsize=8,color='r', weight='bold')
    ...
    plt.grid()
figname='savage_PEHaplo_contig_alignments2.pdf'
